...
class X_days_after_entry_created_postmaster_notification extends Base_notification {
	...
	public $title = 'X Days After Entry Created';
	...
	public $description = 'This notification is used when you want to automatically send emails X days after entry has been created. You can configure as many days as desired after placing of an entry to send an email.';
	...
	protected $fields = array(
		'threshold' => array(
			...
			'description' => 'Enter amount of relative time after the creation date. If the current time is past this relative time, and email will send. <br>Example: "24 hours", "3 days", "1 week"',
		),
		'channel' => array(
			'label' 	  => 'Channel Name',
	...
	protected $tables = array(
		'postmaster_after_entry_created_emails' 	=> array(
			'entry_id'	=> array(
				'type'				=> 'int',
				'constraint'		=> 100,
				'primary_key'		=> TRUE,
	...
	public function send()
	{
		...
		$where = array('entry_date <'  => $this->EE->localize->now);

		$where['entry_date >='] = $this->EE->localize->now + $diff;
        
        $where['channel_titles.entry_id >'] = 20749;
		...
		$entries = $this->EE->channel_data->get_entries(array(
			'select' => 'postmaster_after_entry_created_emails.entry_id IS NOT NULL as \'has_sent\'',
			'where'  => $where,
			'having' => array(
				'has_sent !=' => 1
			),
			'left join'  => array('postmaster_after_entry_created_emails' => 'channel_titles.entry_id = postmaster_after_entry_created_emails.entry_id')
		));
		...
		{	
            ...
            $q = $this->EE->db->select('child_id')->from('relationships')->where('parent_id', $entry->entry_id)->where('field_id', 63)->get();
            $parse_vars['course_id'] = $q->row('child_id');
            
            $q = $this->EE->db->select('email')->from('members')->where('member_id', $entry->author_id)->get();
            $parse_vars['email'] = $q->row('email');
	...
	private function _update_entry($id, $data = array())
	{
		...
		$this->EE->db->update('postmaster_after_entry_created_emails', $data);
	}
	...
	private function _insert_entry($id, $data = array())
	{
		...
		$this->EE->db->insert('postmaster_after_entry_created_emails', $data);
	}
	...
	private function _existing_entry($id)
	{
		...
		$data = $this->EE->db->get('postmaster_after_entry_created_emails');
